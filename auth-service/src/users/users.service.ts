...
export class UsersService {
  ...
  async create(signUpUserDto: SignUpUserDto, request: Request) {
    ...
    createdUser.activationKey = this.generateActivationToken(createdUser._id);
    ...
    try {
      ...
      this.eventBus.emit('MAIL_SERVICE_CONFIRMATION_EMAIL', {
        ...
        metadata: {
          token: createdUser.activationKey,
  ...
  async activateUser(token) {
    const user = await this.userRepository.findOneAndUpdate(
      { activationKey: token },
      { $set: { activationKey: '', activated: true } },
    );
  ...
  async resendActivationEmail(user) {
    ...
    this.eventBus.emit('MAIL_SERVICE_ACTIVATION_EMAIL', {
      ...
      metadata: {
        token: updatedUser.activationKey,
